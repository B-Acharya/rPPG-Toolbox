...
_C.LOO.DATA.FILE_LIST_PATH = os.path.join(
    _C.UNSUPERVISED.DATA.CACHED_PATH, "DataFileLists"
)
_C.LOO.DATA.DATASET = ""
...
def update_config(config, args):
    ...
    if (
        not config.TEST.USE_LAST_EPOCH
        and config.VALID.DATA.DATASET is not None
        or config.TOOLBOX_MODE == "ENRICH"
    ):
        # UPDATE VALID PATHS
    ...
    if config.LOO.DATA.FILE_LIST_PATH == default_LOO_FILE_LIST_PATH:
        config.LOO.DATA.FILE_LIST_PATH = os.path.join(
            config.LOO.DATA.CACHED_PATH, "DataFileLists"
        )

    if config.LOO.DATA.EXP_DATA_NAME == "":
        config.LOO.DATA.EXP_DATA_NAME = "_".join(
            [
                config.LOO.DATA.DATASET,
                "SizeW{0}".format(str(config.LOO.DATA.PREPROCESS.RESIZE.W)),
                "SizeH{0}".format(str(config.LOO.DATA.PREPROCESS.RESIZE.H)),
                "ClipLength{0}".format(str(config.LOO.DATA.PREPROCESS.CHUNK_LENGTH)),
                "DataType{0}".format("_".join(config.LOO.DATA.PREPROCESS.DATA_TYPE)),
    ...
    config.LOO.DATA.CACHED_PATH = os.path.join(
        config.LOO.DATA.CACHED_PATH, config.LOO.DATA.EXP_DATA_NAME
    )

    name, ext = os.path.splitext(config.LOO.DATA.FILE_LIST_PATH)
    if not ext:  # no file extension
        config.LOO.DATA.FILE_LIST_PATH = os.path.join(
            config.LOO.DATA.FILE_LIST_PATH,
            config.LOO.DATA.EXP_DATA_NAME
            + "_"
            + str(config.LOO.DATA.BEGIN)
            + "_"
            + str(config.LOO.DATA.END)
            + ".csv",
        )
    elif ext != ".csv":
        raise ValueError(
            "LOO dataset FILE_LIST_PATH must either be a directory path or a .csv file name"
        )

    if ext == ".csv" and config.LOO.DATA.DO_PREPROCESS:
        raise ValueError(
            "User specified LOO dataset FILE_LIST_PATH .csv file already exists. \
                         Please turn DO_PREPROCESS to False or delete existing LOO dataset FILE_LIST_PATH .csv file."
        )

    config.LOG.PATH = os.path.join(config.LOG.PATH, config.VALID.DATA.EXP_DATA_NAME)

    config.MODEL.MODEL_DIR = os.path.join(
        config.MODEL.MODEL_DIR, config.TRAIN.DATA.EXP_DATA_NAME
    )
    config.freeze()
    return
