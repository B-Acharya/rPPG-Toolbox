...
class SCAMPSLoader(BaseLoader):
    ...
    def __init__(self, name, data_path, config_data):
        ...
        self.file_list_path = (
            config_data.FILE_LIST_PATH.split(".")[0]
            + "_"
            + self.dataset_name
            + os.path.basename(config_data.FILE_LIST_PATH)
        )  # append split name before .csv ext
    ...
    def split_raw_data(self, data_dirs, begin, end):
        ...
        if (
            begin == 0 and end == 1
        ):  # return the full directory if begin == 0 and end == 1
            return data_dirs
    ...
    def preprocess_dataset_backup(self, data_dirs, config_preprocess):
        ...
        for i in pbar:
            ...
            frames_clips, bvps_clips = self.preprocess(frames, bvps, config_preprocess)
            self.preprocessed_data_len += self.save(
                frames_clips, bvps_clips, data_dirs[i]["index"]
            )
