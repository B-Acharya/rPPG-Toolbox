...
import cv2
import torch
from rPPG_Toolbox.dataset.data_loader.face_detector.model.yolo import Model
...
class YOLO5Face(object):
        ...
        if device != None:
            if device == "cpu":
                ...
                self.device = torch.device(
                    dev_list[0]
                )  # currently toolbox only supports 1 GPU
            else:
                self.device = torch.device("cpu")
        else:
    ...
    def detect_face(self, frame):
        ...
        imgsz = check_img_size(
            self.img_size, s=self.model.stride.max()
        )  # check img_size
                ...
                for i, det in enumerate(pred):  # detections per image
                    if i == 0 and len(det):
                        ...
                        det[:, 5:15] = scale_coords_landmarks(
                            img.shape[2:], det[:, 5:15], frame.shape
                        ).round()
