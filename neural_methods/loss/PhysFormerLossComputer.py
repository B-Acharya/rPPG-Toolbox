...
class TorchLossComputer(object):
    ...
    def compute_complex_absolute_given_k(output, k, N):
        ...
        hanning = torch.autograd.Variable(
    ...
    def complex_absolute(output, Fs, bpm_range=None):
        ...
        complex_absolute = TorchLossComputer.compute_complex_absolute_given_k(
            output, k, N
        )

        return (
            1.0 / complex_absolute.sum()
        ) * complex_absolute  # Analogous Softmax operator
    ...
    def cross_entropy_power_spectrum_loss(inputs, target, Fs):
        ...
        return F.cross_entropy(
            complex_absolute, target.view((1)).type(torch.long)
    ...
    def cross_entropy_power_spectrum_DLDL_softmax2(inputs, target, Fs, std):
        target_distribution = [
            normal_sampling(int(target), i, std) for i in range(40, 180)
        ]
        ...
        if target - bpm_range[0].view(1).type(torch.long) < 0:
            return (
                ...
                F.cross_entropy(ca, (target).view(1).type(torch.long)),
                torch.abs(target[0] - bpm_range[0] - whole_max_idx),
            )
        else:
            return (
                loss_distribution_kl,
                F.cross_entropy(ca, (target - bpm_range[0]).view(1).type(torch.long)),
                torch.abs(target[0] - bpm_range[0] - whole_max_idx),
            )
